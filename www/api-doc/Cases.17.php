...
<div class="apiDocu-container apiDocu-index">
	...
	<div class="apiDocu-container">
		<div class="apiDocu-url">
			...
			<div class="apiDocu-description apiDocu-description-main">Verifies pending case disputation<br />Apart from case ID following parameters are expected (and mandatory) in POST params:<br /> - email - non-empty e-mail<br /> - code - non-empty validation code<br />Outcome:<br /><br><pre class="apiDocu-json">    {<br />        <span class="apiDocu-string">"result"</span>: <span class="apiDocu-string">"success"</span><br />    }<br /></pre><br />Potential results of disputing:<br /> - <b>invalid_input</b> when input is invalid<br /> - <b>expired</b> when validation request is expired<br /> - <b>no_request</b> when no such request found<br /> - <b>already_validates</b> when the request was already validates<br /> - <b>inconsistent_already_final</b> when at least of one taggings has final flag (was added meanwhile)<br /> - <b>inconsistent_changed_meanwhile</b> when at least of of the taggings is differing from disputed state<br /> - <b>success</b> when succeeded<br /> - <b>fail</b> when other error state happens</div>
